    ...
    [
        ...
        'value' => function ($model) {
            ...
            foreach ($model->customersAndGuarantor as $value) {
                ...
                $customer_job = $value->jobsName($value->job_title);
                $customer_id_number = $value->id_number;
                ...
                if (!empty($judicaryAction)) {
                    array_push($names_array, $value->name . '(' . $judicaryAction->name . ')');

                } else {
                    array_push($names_array, $value->name . '(لا إجرائات متبعه)');
                }
                array_push($names_array, ' ' . '(' .$customer_job . ') // ');
                array_push($names_array, ' ' . '(' .$customer_id_number . ') // ');
                array_push($names_array, ' ' . '-' .$customer_id . '- // ');
